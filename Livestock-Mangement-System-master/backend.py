...
if not (os.path.isfile('data.db')):
    ...
    with open('data.db', 'w') as f:
        ...
        c.execute('CREATE TABLE MasterTable(' +
                  createStmt_MasterDatabase + ')')
        conn.commit()
        ...
        c.execute('CREATE TABLE LivestockNetworth(' +
                  createStmt_LivestockNetworth + ')')
        conn.commit()
        ...
        values = {'goat_id': 1, 'breed': 'some', 'date_of_birth': date(
            2001, 12, 21), 'gender': 'm', 'pregnant': 'No', 'weight': 10, 'no_of_kids': 0, 'no_of_male_kids': 0, 'no_of_female_kids': 0, 'mortality': 'Alive'}
        ...
        for i in range(0, 50):
            ...
            c.execute('SELECT (julianday(:curdate) - julianday(:date_of_birth)) AS day WHERE day > 365 ',
                      {'curdate': datetime.date(datetime.now()), 'date_of_birth': values['date_of_birth']})
            r = c.fetchall()
            age = 'Kid' if len(r) == 0 else 'Adult'
            # Determining the category of the new goat
            category = age + \
                ' Male' if values['gender'] == 'm' else age + ' Female'

            if not exists:
                # Inserting the new record in LivestockNetworth
                c.execute('INSERT INTO LivestockNetworth(category, breed, cost, total_weight, total_cost) VALUES(:category, :breed, 0, :total_weight, 0)', {
                          'category': category, 'breed': values['breed'], 'total_weight': values['weight']})
                conn.commit()
            else:
                for res in result:
                    ...
                    if res[0] == values['breed'] and res[1] == category:
                        ...
                        c.execute('SELECT total_weight FROM LivestockNetworth WHERE breed=:breed AND category=:category', {
                                  'breed': res[0], 'category': res[1]})
                        total_weight = c.fetchall()[0][0] + values['weight']

                        # Update total weight in LivestockNetworth
                        c.execute('UPDATE LivestockNetworth SET total_weight=:total_weight', {
...
def updateLiveStockNetworth(values):
    ...
    # Get types of breeds present in LivestockNetworth
    c.execute(
        'SELECT breed, category FROM LivestockNetworth GROUP BY breed, category')
    result = c.fetchall()
    if len(result) != 0:
        exists = True
    ...
    if not exists:
        ...
        for res in result:
            ...
            if res[0] == values['breed'] and res[1] == category:
                ...
                total_weight = c.fetchall()[0][0] + values['weight']
                # Update total weight in LivestockNetworth
                c.execute('UPDATE LivestockNetworth SET total_weight=:total_weight WHERE breed=:breed AND category=:category', {
                          'total_weight': total_weight, 'breed': res[0], 'category': res[1]})
                conn.commit()
        else:
            # Inserting the new record in LivestockNetworth
            c.execute('INSERT INTO LivestockNetworth(category, breed, cost, total_weight, total_cost) VALUES(:category, :breed, 0, :total_weight, 0)', {
...
if datetime.date(datetime.now()) == date(date.today().year, 4, 1):
    ...
    c.execute(
        'DELETE FROM MasterTable WHERE sold_rate!=\'None\' OR mortality=\'Dead\'')
    c.execute('DELETE FROM LivestockNetworth')

    c.execute(
        'SELECT goat_no, breed, date_of_birth, weight, gender FROM MasterTable')
    result = c.fetchall()
    for res in result:
        values = {'goat_id': res[0], 'breed': res[1],
                  'date_of_birth': res[2], 'weight': res[3], 'gender': res[4]}
...
class DataBase:
    ...
    def getColumnNames(self):
        names = ['goat_no', 'breed', 'date_of_birth', 'gender', 'pregnant', 'weight', 'date_of_delivery', 'no_of_kids', 'no_of_male_kids', 'no_of_female_kids', 'mortality',
                 'Anthrax', 'Haemorrhagic Septicemia', 'Enterotoxaemia', 'Black Quarter', 'P.P.R.', 'Foot and Mouth Disease', 'Goat Pox', 'C.C.P.P', 'sold_date', 'sold_rate']
        return names
    ...
    def insertGoatRecord(self, values, mother_id):
        try:
            if mother_id == 'null':
                ...
                c.execute('INSERT INTO KidsTable VALUES(:mother_id, :kid_id, :gender)', {
                          'mother_id': mother_id, 'kid_id': values['goat_id'], 'gender': values['gender']})

                # Updating number of kids
                c.execute('SELECT no_of_kids, no_of_male_kids, no_of_female_kids FROM MasterTable WHERE goat_no=:goat_id', {
                          'goat_id': mother_id})
                res = c.fetchall()
                res = list(res[0]) if len(res) != 0 else [0, 0, 0]
                res[0] += 1

                c.execute('UPDATE MasterTable SET no_of_kids=:no_of_kids WHERE goat_no=:mother_id', {
                          'no_of_kids': res[0], 'mother_id': mother_id})

                if values['gender'] == 'm':
                    res[1] += 1
                    c.execute('UPDATE MasterTable SET no_of_male_kids=:no_of_male_kids WHERE goat_no=:mother_id', {
                              'no_of_male_kids': res[1], 'mother_id': mother_id})
                elif values['gender'] == 'f':
                    res[2] += 1
                    c.execute('UPDATE MasterTable SET no_of_female_kids=:no_of_female_kids WHERE goat_no=:mother_id', {
                              'no_of_female_kids': res[2], 'mother_id': mother_id})

            # Updating mortality
            c.execute('UPDATE MasterTable SET mortality=\'Alive\' WHERE goat_no=:goat_id', {
                      'goat_id': values['goat_id']})
    ...
    def getMaleKidID(self, mother_id):
        c.execute('SELECT kid_id FROM KidsTable WHERE mother_id = ' +
                  mother_id + ' AND gender = \'m\'')
        res = c.fetchall()
        return res

    def getFemaleKidID(self, mother_id):
        c.execute('SELECT kid_id FROM KidsTable WHERE mother_id = ' +
                  mother_id + ' AND gender = \'f\'')
        res = c.fetchall()
        return res

    def getMotherId(self, goat_no):
        c.execute('SELECT mother_id FROM KidsTable WHERE kid_id=:goat_id', {
                  'goat_id': goat_no})
        res = c.fetchall()
        return res[0][0]
    ...
    def updateGoatRecord(self, goatValues, isSold, isWeightUpdated):
        ...
        c.execute(
            'UPDATE MasterTable SET mortality=:mortality, breed=:breed, date_of_birth=:dob WHERE goat_no=:goat_id', goatValues)
        conn.commit()

        if isSold:
            c.execute(
                'UPDATE MasterTable SET sold_date=:sold_date, sold_rate=:sold_rate WHERE goat_no=:goat_id', goatValues)

        if isWeightUpdated:
            c.execute(
                'UPDATE MasterTable SET weight=:weight WHERE goat_no=:goat_id', goatValues)

        if goatValues['pregnant'] == 'Yes':
            date_of_delivery = str(datetime.date(
                datetime.now()) + relativedelta(months=+5))
            c.execute('UPDATE MasterTable SET pregnant=:pregnant, date_of_delivery=:date_of_delivery WHERE goat_no=:goat_id', {
                      'pregnant': goatValues['pregnant'], 'date_of_delivery': date_of_delivery, 'goat_id': goatValues['goat_id']})

        conn.commit()

        if isWeightUpdated:
            c.execute('INSERT INTO WeightTable' + str(goatValues['goat_id']) + ' values(:weight, :date_checked)', {
                      'weight': goatValues['weight'], 'date_checked': str(datetime.date(datetime.now()))})
            conn.commit()
    ...
    def updateVaccination(self, vacc_no, goat_id):
        if vacc_no == 1:
            c.execute('UPDATE MasterTable SET v1=? WHERE goat_no=?',
                      (None, goat_id))
        if vacc_no == 2:
            c.execute('UPDATE MasterTable SET v2=? WHERE goat_no=?',
                      (None, goat_id))
        if vacc_no == 3:
            c.execute('UPDATE MasterTable SET v3=? WHERE goat_no=?',
                      (None, goat_id))
        if vacc_no == 4:
            c.execute('UPDATE MasterTable SET v4=? WHERE goat_no=?',
                      (None, goat_id))
        if vacc_no == 5:
            c.execute('UPDATE MasterTable SET v5=? WHERE goat_no=?',
                      (None, goat_id))
        if vacc_no == 6:
            c.execute('UPDATE MasterTable SET v6=? WHERE goat_no=?',
                      (None, goat_id))
        if vacc_no == 7:
            c.execute('UPDATE MasterTable SET v7=? WHERE goat_no=?',
                      (None, goat_id))
        if vacc_no == 8:
            c.execute('UPDATE MasterTable SET v8=? WHERE goat_no=?',
                      (None, goat_id))
        conn.commit()
    ...
    def getGoatsToBeVaccinated(self, vacc_no):
        ...
        if vacc_no == 1 or vacc_no == 2 or vacc_no == 4:
            ...
            c.execute('SELECT goat_no, v' + str(vacc_no) + ', date_of_birth FROM MasterTable WHERE (julianday(:curdate) - julianday(date_of_birth) <= 365) AND mortality = "Alive"',
                      {'curdate': datetime.date(datetime.now())})
            res = c.fetchall()
            for rec in res:
                if rec[1] != None:
                    newd = datetime.strptime(
                        rec[1], '%Y-%m-%d').date() + relativedelta(months=+6)
                else:
                    newd = datetime.strptime(
                        rec[2], '%Y-%m-%d').date() + relativedelta(months=+6)
                g_no = rec[0]
            ...
            for rec in res:
                if rec[1] != None:
                    newd = datetime.strptime(
                        rec[1], '%Y-%m-%d').date() + relativedelta(years=1)
                else:
            ...
            for rec in res:
                if rec[1] != None:
                    newd = datetime.strptime(
                        rec[1], '%Y-%m-%d').date() + relativedelta(months=+4)
                else:
                    newd = datetime.strptime(
                        rec[2], '%Y-%m-%d').date() + relativedelta(months=+4)
                g_no = rec[0]
                # if datetime.today().month == newd.month and datetime.today().year == newd.year:
                dates.append([g_no, str(newd)])
            ...
            for rec in res:
                if rec[1] != None:
                    newd = datetime.strptime(
                        rec[1], '%Y-%m-%d').date() + relativedelta(months=+3)
                else:
                    newd = datetime.strptime(
                        rec[2], '%Y-%m-%d').date() + relativedelta(months=+3)
                g_no = rec[0]
                # if datetime.today().month == newd.month and datetime.today().year == newd.year:
                dates.append([g_no, str(newd)])
            ...
            for rec in res:
                if rec[1] != None:
                    newd = datetime.strptime(
                        rec[1], '%Y-%m-%d').date() + relativedelta(years=3)
                else:
                    dob = datetime.strptime(rec[2], '%Y-%m-%d').date()
                    toBeAdded = date.today() - dob
                    toBeAdded = toBeAdded.days
    ...
    def getVaccinationDates(self, goat_no):
        c.execute('SELECT v1, v2, v3, v4, v5, v6, v7, v8 FROM MasterTable WHERE goat_no=:goat_id', {
                  'goat_id': goat_no})
        res = c.fetchall()
        return res[0]
    ...
    def getBreedReadyGoats(self):
        c.execute('SELECT goat_no, breed FROM MasterTable WHERE (julianday(:curdate) - julianday(date_of_birth) > 365)AND ((julianday(:curdate) - julianday(date_of_delivery) > 90)OR(pregnant=\'No\')) AND(gender=\'f\')',
                  {'curdate': datetime.date(datetime.now())})
        res = c.fetchall()
        return res
        ...
        if dates != None:
            for i in range(len(dates)):
                if datetime.strptime(dates[i][1], '%Y-%m-%d').date() <= datetime.date(datetime.now()):
                    c.execute('UPDATE MasterTable SET v' + str(vacc_no) + '=:curdate WHERE goat_no=:goat_id', {
                              'curdate': str(datetime.date(datetime.now())), 'goat_id': dates[i][0]})
            conn.commit()

    # For View goat
    def getKidsTableData(self, goat_no):
        c.execute('SELECT * FROM KidsTable WHERE mother_id=:goat_id',
                  {'goat_id': goat_no})
        res = c.fetchall()
        return res
    ...
    def insertLabour(self, labourValues):
        no_of_days = 30
        c.execute(
            'INSERT INTO Labour(category, salary, count) VALUES(:category, :salary, :count)', labourValues)
        monthly_total = int(labourValues['count']) * \
            int(labourValues['salary']) * no_of_days
        c.execute('UPDATE Labour SET monthly_total=:monthly_total WHERE category=:category AND salary=:salary AND count=:count', {
                  'monthly_total': monthly_total, 'category': labourValues['category'], 'salary': labourValues['salary'], 'count': labourValues['count']})
        conn.commit()
    ...
    def deleteLabourRecord(self, labourValues):
        c.execute(
            'DELETE FROM Labour WHERE category=:category AND salary=:salary AND count=:count', labourValues)
        conn.commit()
    ...
    def insertFeed(self, feedValues):
        ...
        c.execute(
            'INSERT INTO Feed(item, weight, cost) VALUES(:item, :weight, :cost)', feedValues)
        c.execute('UPDATE Feed SET total_cost=:total_cost, purchase_date=:purchase_date WHERE item=:item AND weight=:weight AND cost=:cost', {
                  'total_cost': total_cost, 'item': feedValues['item'], 'purchase_date': datetime.date(datetime.now()), 'cost': feedValues['cost'], 'weight': feedValues['weight']})
        conn.commit()
    ...
    def deleteFeedRecord(self, feedValues):
        c.execute(
            'DELETE FROM Feed WHERE purchase_date=:purchase_date AND item=:item AND weight=:weight AND cost=:cost', feedValues)
        conn.commit()

    # MiscExpense Table
    def insertMiscExpenditure(self, miscExpenseValue):
        c.execute(
            'INSERT INTO MiscExpense(category, cost) VALUES(:category, :cost)', miscExpenseValue)
        c.execute('UPDATE MiscExpense SET purchase_date=:purchase_date WHERE category=:category AND cost=:cost', {
                  'purchase_date': datetime.date(datetime.now()), 'category': miscExpenseValue['category'], 'cost': miscExpenseValue['cost']})
        conn.commit()
    ...
    def insertIncome(self, incomeValues):
        c.execute(
            'INSERT INTO Income(details, cost) VALUES(:details, :cost)', incomeValues)
        c.execute('UPDATE Income SET purchase_date=:purchase_date WHERE details=:details AND cost=:cost', {
                  'purchase_date': datetime.date(datetime.now()), 'details': incomeValues['details'], 'cost': incomeValues['cost']})
        conn.commit()
    ...
    def deleteIncomeRecord(self, incomeValues):
        c.execute(
            'DELETE FROM Income WHERE purchase_date=:purchase_date AND details=:details AND cost=:cost', incomeValues)
        conn.commit()
    ...
    def getKidCount(self):
        c.execute('SELECT count(goat_no),gender FROM MasterTable where (julianday(:dob) - julianday(date_of_birth) ) <= 365 GROUP BY gender',
                  {'dob': datetime.date(datetime.now())})
        return c.fetchall()

    def getDeadCount(self):
        c.execute(
            'SELECT count(goat_no),gender FROM MasterTable where mortality=\'Dead\' GROUP BY gender')
        return c.fetchall()

    def getTotalSoldGoatsRate(self):
        c.execute(
            'SELECT SUM(sold_rate) FROM MasterTable WHERE sold_rate != \'None\'')
        res = c.fetchall()
